...
const Auth: React.FC<AuthProps> = ({ onClose, onSuccess }) => {
  ...
  const [name, setName] = useState(localStorage.getItem('') || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let response;
      if (isLogin) {
        response = await fetch('http://localhost:5002/api/auth/login', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ email, password }),
        });
      } else {
        response = await fetch('http://localhost:5002/api/auth/register', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok) {
        if (!isLogin) {
          // Store user name and switch to login mode
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
      <div className="bg-gray-800 rounded-lg p-6 w-full max-w-md">
        <h2 className="text-2xl font-bold mb-6">
          {isLogin ? `Welcome, ${name || 'Guest'}! Sign In` : 'Create Account'}
        </h2>
