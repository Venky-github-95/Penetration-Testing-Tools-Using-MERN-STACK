import { Shield, Search, Menu, X, User, Bug, PieChart, Globe, FileMinus, ShieldAlert, LogOut , RefreshCw , Terminal , FileBarChart} from 'lucide-react';
...
function App() {
  ...
  const [currentPage, setCurrentPage] = useState<string>(
    localStorage.getItem('currentPage') || 'scan'
  );


  const [pageKeys, setPageKeys] = useState<{ [key: string]: number }>({
      scan: Date.now(),
      owasp: Date.now(),
      virustotal: Date.now(),
      tools: Date.now(),
    ...
    // Function to refresh only the current page component
  const refreshPage = (): void => {
    setPageKeys((prevKeys) => ({
      ...prevKeys,
      [currentPage]: Date.now(), // Update key only for the current page
    }));
  };
...
<div>
      {/* <button onClick={refreshPage}>Refresh Current Page</button> */}
      <button
      onClick={refreshPage}
      className="px-4 py-2 bg-blue-600 text-white rounded-md shadow-lg hover:bg-blue-700 transition duration-100 ease-in-out flex items-center justify-center fixed top-4 right-4">
      <RefreshCw className="w-6 h-6 animate-spin" />
    </button>

      <div style={{ display: currentPage === 'scan' ? 'block' : 'none' }}>
        <VulnerabilityScan key={pageKeys.scan} />
      </div>

      <div style={{ display: currentPage === 'owasp' ? 'block' : 'none' }}>
        <Owasp key={pageKeys.owasp} />
      </div>

      <div style={{ display: currentPage === 'virustotal' ? 'block' : 'none' }}>
        <Virustotal key={pageKeys.virustotal} />
      </div>

      <div style={{ display: currentPage === 'tools' ? 'block' : 'none' }}>
        <ToolComparison key={pageKeys.tools} />
      </div>

      <div style={{ display: currentPage === 'reports' ? 'block' : 'none' }}>
        <SecurityReports key={pageKeys.reports} />
      </div>

      <div style={{ display: currentPage === 'urlscan' ? 'block' : 'none' }}>
        <Urlscan key={pageKeys.urlscan} />
      </div>
    </div>
